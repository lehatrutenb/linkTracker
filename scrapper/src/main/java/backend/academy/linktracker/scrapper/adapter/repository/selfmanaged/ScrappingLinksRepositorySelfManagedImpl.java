...
public class ScrappingLinksRepositorySelfManagedImpl implements ScrappingLinksRepository {
    ...
    public Collection<ScrapperLink> readAllScrapperLinks() {
        return client.sql("SELECT * FROM scrapper_link").query(new ScrapperLinkEntityRowMapper()).list().stream()
    ...
    public ScrapperLink createScrapperLink(ScrapperLink scrapperLink) {
        ...
        return client.sql(
                        "INSERT INTO scrapper_link (link_id, link_uri, updated_at) VALUES (nextval('link_id_seq'), :link_uri, :updated_at) RETURNING *")
    ...
    public ScrapperLink updateScrapperLink(ScrapperLink scrapperLink) {
        var entity = new ScrapperLinkEntity(scrapperLink);
        return client.sql(
                        "UPDATE scrapper_link SET link_uri = :link_uri, updated_at = :updated_at WHERE link_id = :link_id RETURNING *")
