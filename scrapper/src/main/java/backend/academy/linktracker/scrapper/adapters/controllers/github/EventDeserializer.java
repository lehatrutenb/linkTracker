...
public class EventDeserializer extends ValueDeserializer<Event> {
    private final EventPayloadDeserializer payloadDeserializer = new EventPayloadDeserializer();
    ...
    public Event deserialize(JsonParser p, DeserializationContext ctxt) throws JacksonException {
        ...
        event.setType(rootNode.get("type").asString());
        ...
        event.setOrg(readContext.readValue(rootNode.get("org").traverse(ctxt), Actor.class));
        event.setRepo(readContext.readValue(rootNode.get("repo").traverse(ctxt), EventRepo.class));
        event.setCreatedAt(readContext.readValue(rootNode.get("created_at").traverse(ctxt), OffsetDateTime.class));
    ...
    //    public EventPayload deserializePayload(JsonParser jsonParser, String type,
    //                                           DeserializationContext ctxt) throws JacksonException {
    //        payloadDeserializer.setType(type);
    //        return payloadDeserializer.deserialize(jsonParser, ctxt);
    //    }
}
